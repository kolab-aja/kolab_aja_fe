  ...
  <v-container class="custom-container d-flex flex-column align-center py-10">
    ...
    <v-row class="gap-4">
      <v-col v-for="item in roleList" :key="item.roleId" cols="12" md="4">
        <v-card
          :class="{ 'active-card': selectedRoleId === item.roleId }"
          class="pa-5 border role-card"
          variant="flat"
        ...
        >
          <v-card-title class="d-flex justify-left pb-6">
            <v-icon
              :icon="item.icon"
              size="x-large"
              :color="selectedRoleId === item.roleId ? 'white' : ''"
            ></v-icon>
          </v-card-title>
          <v-card-title
            :class="{ 'text-white': selectedRoleId === item.roleId }"
            class="text-h5"
            >{{ item.name }}</v-card-title
          >
          <v-card-text
            :class="selectedRoleId === item.roleId ? 'role-desc-active' : 'role-desc'"
          >
            {{ item.description }}
          </v-card-text>
        </v-card>
